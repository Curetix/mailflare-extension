...
export function useCloudflare() {
  ...
  const isDataLoading = useMemo(
    () => zones.isPending || emailDestinations.isPending || emailRules.isPending,
    [zones.isPending, emailDestinations.isPending, emailRules.isPending],
  );

  const isMutationLoading = useMemo(
    () => createEmailRule.isPending || updateEmailRule.isPending || deleteEmailRule.isPending,
    [createEmailRule.isPending, updateEmailRule.isPending, deleteEmailRule.isPending],
  );

  const isLoading = useMemo(
    () => isDataLoading || isMutationLoading,
    [isDataLoading, isMutationLoading],
  );
